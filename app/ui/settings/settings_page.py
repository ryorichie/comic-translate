...
class SettingsPage(QtWidgets.QWidget):
    ...
    def get_export_settings(self):
        ...
        for file_type in self.ui.from_file_types:
            settings["save_as"][f".{file_type}"] = self.ui.export_widgets[
                f".{file_type}_save_as"
            ].currentText()
        return settings
                ...
                creds.update(
                    {
                        ...
                        "api_key_translator": _text_or_none(
    ...
    def import_font(self, file_paths: List[str]):
        ...
        if file_paths:
            ...
            font_files = [
                os.path.join(font_folder_path, f)
    ...
    def select_color(self, outline=False):
        default_color = (
            QtGui.QColor("#000000") if not outline else QtGui.QColor("#FFFFFF")
        )
        ...
        if color_dialog.exec() == QtWidgets.QDialog.Accepted:
            ...
            if color.isValid():
                button = (
                    ...
                    if not outline
                    else self.ui.outline_color_button
                )
        ...
        if save_keys:
            for service, cred in credentials.items():
                ...
                if translated_service == "Microsoft Azure":
                    settings.setValue(
                        f"{translated_service}_api_key_ocr", cred["api_key_ocr"]
                    )
                    settings.setValue(
                        f"{translated_service}_api_key_translator",
                        cred["api_key_translator"],
                    )
                    settings.setValue(
                        f"{translated_service}_region_translator",
                        cred["region_translator"],
                    )
                    settings.setValue(
                        f"{translated_service}_endpoint", cred["endpoint"]
                    )
            ...
            settings.remove(
                "credentials"
            )  # Clear all credentials if save_keys is unchecked
        settings.endGroup()
    ...
    def load_settings(self):
        ...
        if strategy == "Resize":
            self.ui.resize_spinbox.setValue(
                settings.value("resize_limit", 960, type=int)
            )
        elif strategy == "Crop":
            self.ui.crop_margin_spinbox.setValue(
                settings.value("crop_margin", 512, type=int)
            )
            self.ui.crop_trigger_spinbox.setValue(
                settings.value("crop_trigger_size", 512, type=int)
            )
        ...
        self.ui.image_checkbox.setChecked(
            settings.value("image_input_enabled", False, type=bool)
        )
        ...
        self.ui.raw_text_checkbox.setChecked(
            settings.value("export_raw_text", False, type=bool)
        )
        ...
        for file_type in [".pdf", ".epub", ".cbr", ".cbz", ".cb7", ".cbt"]:
            self.ui.export_widgets[f"{file_type}_save_as"].setCurrentText(
                settings.value(file_type, file_type[1:])
            )
            ...
            for service in self.ui.credential_services:
                ...
                if translated_service == "Microsoft Azure":
                    self.ui.credential_widgets["Microsoft Azure_api_key_ocr"].setText(
                        settings.value(f"{translated_service}_api_key_ocr", "")
                    )
                    self.ui.credential_widgets[
                        "Microsoft Azure_api_key_translator"
                    ].setText(
                        settings.value(f"{translated_service}_api_key_translator", "")
                    )
                    self.ui.credential_widgets["Microsoft Azure_region"].setText(
                        settings.value(f"{translated_service}_region_translator", "")
                    )
                    self.ui.credential_widgets["Microsoft Azure_endpoint"].setText(
                        settings.value(f"{translated_service}_endpoint", "")
                    )
                elif translated_service == "Custom":
                    self.ui.credential_widgets[f"{service}_api_key"].setText(
                        settings.value(f"{translated_service}_api_key", "")
                    )
                    self.ui.credential_widgets[f"{service}_api_url"].setText(
                        settings.value(f"{translated_service}_api_url", "")
                    )
                    self.ui.credential_widgets[f"{service}_model"].setText(
                        settings.value(f"{translated_service}_model", "")
                    )
                else:
                    self.ui.credential_widgets[f"{service}_api_key"].setText(
                        settings.value(f"{translated_service}_api_key", "")
    ...
    def show_restart_dialog(self):
        ...
        msg_box.setText(
            self.tr(
                "Please restart the application for the language changes to take effect."
            )
        )
    ...
    def add_font_family(self, font_input: str) -> QFont:
        ...
        if os.path.splitext(font_input)[1].lower() in [
            ".ttf",
            ".ttc",
            ".otf",
            ".woff",
