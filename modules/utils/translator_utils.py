...
def get_raw_translation(blk_list: List[TextBlock]):
    ...
    raw_translations_json = json.dumps(
        rw_translations_dict, ensure_ascii=False, indent=4
    )
...
def format_translations(
    blk_list: List[TextBlock], trg_lng_cd: str, upper_case: bool = True
):
    for blk in blk_list:
        translation = blk.translation
        if any(lang in trg_lng_cd.lower() for lang in ["zh", "ja", "th"]):
            ...
            translation = "".join(
                word if word in [".", ","] else f" {word}" for word in seg_result
            ).lstrip()
            blk.translation = translation
        else:
