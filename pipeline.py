...
class ComicTranslatePipeline:
    def __init__(self, main_page):
        ...
        self.translation_cache = (
            {}
        )  # Translation results cache: {(image_hash, translator_key, source_lang, target_lang, extra_context): {block_id: {source_text: str, translation: str}}}
    ...
    def clear_translation_cache(self):
        ...
        self.image_loader = ImageStateController(main_page)
    ...
    def load_box_coords(self, blk_list: List[TextBlock]):
        ...
        if self.main_page.image_viewer.hasPhoto() and blk_list:
            for blk in blk_list:
                ...
                if blk.tr_origin_point:
                    rect_item.setTransformOriginPoint(
                        QtCore.QPointF(*blk.tr_origin_point)
                    )
            ...
            rect = self.main_page.rect_item_ctrl.find_corresponding_rect(
    ...
    def _generate_image_hash(self, image):
        ...
        try:
            ...
            shape_str = (
                str(image.shape) if hasattr(image, "shape") else str(type(image))
            )
            fallback_data = (
                shape_str.encode() + str(image.dtype).encode()
            ...
            for cached_id in cached_results.keys():
                try:
                    ...
                    if len(parts) >= 5:
                        ...
                        ):  # 1 degree tolerance for angle

                            logger.debug(
                                f"Fuzzy match found for OCR: {target_id[:20]}... -> {cached_id[:20]}..."
                            )
                ...
                try:
                    ...
                    if len(parts) >= 5:
                        ...
                        if (
                            ...
                            logger.debug(
                                f"Fuzzy match found for translation: {target_id[:20]}... -> {cached_id[:20]}..."
                            )
                            return cached_id, cached_results[cached_id]
                except (ValueError, IndexError):
                    continue
            ...
            if processed_blk_list is not None:
                for original_blk, processed_blk in zip(blk_list, processed_blk_list):
                    block_id = self._get_block_id(
                        original_blk
                    )  # Use original block for ID
                    text = (
                        getattr(processed_blk, "text", "") or ""
                    )  # Get text from processed block
        ...
        try:
            ...
            if processed_blk_list is not None:
                for original_blk, processed_blk in zip(blk_list, processed_blk_list):
                    ...
                    translation = (
                        getattr(processed_blk, "translation", "") or ""
                    )  # Get translation from processed block
                    source_text = (
                        getattr(original_blk, "text", "") or ""
                    )  # Get source text from original block
        ...
        if matched_id is not None:  # Block found in cache
            if result:  # Block has cached data
                ...
                if cached_source_text == current_source_text:
                    ...
                    logger.debug(
                        f"Cache invalid: source text changed from '{cached_source_text}' to '{current_source_text}'"
                    )
    ...
    def _can_serve_all_blocks_from_translation_cache(self, cache_key, block_list):
        ...
        for block in block_list:
            # Use the centralized cache retrieval function
            cached_translation = self._get_cached_translation_for_block(
                cache_key, block
            )
            if (
                cached_translation is None
            ):  # Block not found in cache or source text changed
                return False
    ...
    def _apply_cached_translations_to_blocks(self, cache_key, block_list):
        ...
        for block in block_list:
            ...
            ):  # Block found in cache and source text matches
    ...
    def OCR_image(self, single_block=False):
        ...
        if (
            ...
            and self.main_page.image_viewer.rectangles
        ):
            image = self.main_page.image_viewer.get_cv2_image()
            ...
            if single_block:
                ...
                if self._is_ocr_cached(cache_key):
                    ...
                    if (
                    ...
                    ):  # Block was processed before (even if text is empty)
                        blk.text = cached_text
                        logger.info(
                            f"Using cached OCR result for block: '{cached_text}'"
                        )
                        return
                    else:
                        logger.info(
                            "Block not found in cache, processing single block..."
                        )
                        ...
                        logger.info(
                            f"Processed single block and updated cache: '{blk.text}'"
                        )
                else:
                    # Run OCR on all blocks and cache the results
                    logger.info(
                        "No cached OCR results found, running OCR on entire page..."
                    )
                    self.ocr.initialize(self.main_page, source_lang)
                    ...
                    if all_blocks_copy:
                        ...
                        self._cache_ocr_results(
                            cache_key, self.main_page.blk_list, all_blocks_copy
                        )
                        cached_text = self._get_cached_text_for_block(cache_key, blk)
                        blk.text = cached_text
                        logger.info(
                            f"Cached OCR results and extracted text for block: {cached_text}"
                        )
            else:
                # For full page OCR, check if we can use cached results
                if self._can_serve_all_blocks_from_ocr_cache(
                    cache_key, self.main_page.blk_list
                ):
                    # All blocks can be served from cache
                    self._apply_cached_ocr_to_blocks(cache_key, self.main_page.blk_list)
                    logger.info(
                        f"Using cached OCR results for all {len(self.main_page.blk_list)} blocks"
                    )
                else:
                    ...
                    if self.main_page.blk_list:
                        ...
                        logger.info(
                            "OCR completed and cached for %d blocks",
        ...
        if self.main_page.image_viewer.hasPhoto() and self.main_page.blk_list:
            ...
            if single_block:
                ...
                if self._is_translation_cached(translation_cache_key):
                    ...
                    if (
                        cached_translation is not None
                    ):  # Block was processed and source text matches
                        blk.translation = cached_translation
                        logger.info(
                            f"Using cached translation result for block: '{cached_translation}'"
                        )
                        set_upper_case([blk], upper_case)
                        return
                    else:
                        logger.info(
                            "Block not found in cache or source text changed, processing single block..."
                        )
                    ...
                    if translation_cache_key in self.translation_cache:
                        ...
                        # This shouldn't happen, but handle it gracefully
                        self._cache_translation_results(
                            translation_cache_key, single_block_list
                        )

                    logger.info(
                        f"Processed single block and updated cache: '{blk.translation}'"
                    )
                    set_upper_case([blk], upper_case)
                else:
                    # Run translation on all blocks and cache the results
                    logger.info(
                        "No cached translation results found, running translation on entire page..."
                    )
                    # Create a mapping between original blocks and their copies
                    all_blocks_copy = []
                    ...
                    if all_blocks_copy:
                        ...
                        blk.translation = cached_translation
                        logger.info(
                            f"Cached translation results and extracted translation for block: {cached_translation}"
                        )

                    set_upper_case([blk], upper_case)
            else:
                # For full page translation, check if we can use cached results
                if self._can_serve_all_blocks_from_translation_cache(
                    translation_cache_key, self.main_page.blk_list
                ):
                    # All blocks can be served from cache with matching source text
                    self._apply_cached_translations_to_blocks(
                        translation_cache_key, self.main_page.blk_list
                    )
                    logger.info(
                        f"Using cached translation results for all {len(self.main_page.blk_list)} blocks"
                    )
                else:
                    ...
                    logger.info(
                        "Translation completed and cached for %d blocks",
                        len(self.main_page.blk_list),
                    )
    ...
    def batch_process(self, selected_paths: List[str] = None):
        ...
        image_list = (
            selected_paths if selected_paths is not None else self.main_page.image_files
        )
        ...
        for index, image_path in enumerate(image_list):
            ...
            if blk_list:
                ...
                self.log_skipped_image(
                    directory, timestamp, image_path, "No text blocks detected"
                )
                continue
            ...
            try:
                ...
                if (not raw_text_obj) or (not translated_text_obj):
                    ...
                    self.log_skipped_image(
                        directory, timestamp, image_path, "Translator: empty JSON"
                    )
                    continue
